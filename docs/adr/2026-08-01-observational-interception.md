...
updated: 2026-08-01T03:36:55+09:00
agent: GPT-5 Codex
---
...
- Stopping the final interceptor is retriable. Successfully restored adapters are forgotten, failed restoration handles and the final registration are retained, and `stop()` keeps throwing until every adapter has been restored.
...
- A failed `stop()` leaves the interceptor running so the caller can repair the external condition and retry without stacking another adapter.
